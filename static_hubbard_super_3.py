...
def get_leftVacuum(L_static):
    ...
    row_ind=[]
    for state in states:
        #shifting 2*L to the right => only the 2*L rightmost remain
        spin_up=state >> 2*L
        ...
        if equal_up:      
            ...
            if equal_down:
                ...
                n=int((spin_up_fock.bit_count()+spin_down_fock.bit_count())/2)
...
z=np.zeros(np.shape(vl0))
dif=(vl0-leftVacuum).toarray()
# for i in range(len(dif)):
#     if abs(dif[i])>1e-10:
#         print(i,vl0[i,0],leftVacuum[i,0])
